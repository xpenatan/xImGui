/*-------------------------------------------------------
 * This file was generated by JParser
 *
 * Do not make changes to this file
 *-------------------------------------------------------*/

package imgui;

import com.github.xpenatan.jParser.api.NativeObject;

public class ImVectorImDrawListPtr extends NativeObject {

    private ImDrawList ImDrawList_TEMP_GEN_0;

    static public final ImVectorImDrawListPtr NULL = ImVectorImDrawListPtr.native_new();

    /**
     * Dummy constructor, used internally to creates objects without C++ pointer
     */
    @Deprecated()
    protected ImVectorImDrawListPtr(byte b, char c) {
    }

    /**
     * @return An empty instance without a native address
     */
    public static ImVectorImDrawListPtr native_new() {
        return new ImVectorImDrawListPtr((byte) 0, (char) 0);
    }

    protected void deleteNative() {
        internal_native_deleteNative(native_address);
    }

    public static void internal_native_deleteNative(long this_addr) {
        imgui.natives.JNI_ImVectorImDrawListPtr.internal_native_deleteNative(this_addr);
    }

    public ImDrawList getData(int index) {
        long addr = internal_native_getData_addr(native_address, index);
        if (addr == 0)
            return ImDrawList.NULL;
        if (ImDrawList_TEMP_GEN_0 == null)
            ImDrawList_TEMP_GEN_0 = ImDrawList.native_new();
        ImDrawList_TEMP_GEN_0.internal_reset(addr, false);
        return ImDrawList_TEMP_GEN_0;
    }

    public static long internal_native_getData_addr(long this_addr, int index) {
        return imgui.natives.JNI_ImVectorImDrawListPtr.internal_native_getData_addr(this_addr, index);
    }

    public int size() {
        return internal_native_size(native_address);
    }

    public static int internal_native_size(long this_addr) {
        return imgui.natives.JNI_ImVectorImDrawListPtr.internal_native_size(this_addr);
    }
}
